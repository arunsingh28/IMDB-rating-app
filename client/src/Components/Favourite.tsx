...
const Favourite: React.FC<FavProps> = ({ setOpenFavMenu, changeEvent }) => {
    ...
    const [favMoviesData, setMoviesData] = useState<IMDB[]>()
    ...
    return (
        <div className='h-screen w-72 shadow-2xl shadow-gray-600 bg-white overflow-scroll overflow-x-hidden relative'>
            <div className='fixed top-0 left-0 z-10 bg-white w-full'>
                <button className='bg-blue-500 rounded-md px-5 py-2 font-poppins my-2 mx-2 text-gray-600 font-semibold hover:bg-blue-600' onClick={() => setOpenFavMenu(false)}>
