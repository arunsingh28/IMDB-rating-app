...
const Favourite: React.FC<FavProps> = ({ setOpenFavMenu, changeEvent }) => {
    ...
    const handleRemoveFav = async (id: string, title: string) => {
        ...
        if (isRemove.data) {
            ...
            toast.error(`${title} is already removed from favourites`, {
                position: 'bottom-left',
                theme: 'dark'
            })
        }
            ...
            <div className='mt-16'>
                {
                    isLoading ?
                        ...
                        favMoviesData && favMoviesData.length > 0 ? favMoviesData.map((item) => {
                            return (
                                <div key={item.OMDBID} className='m-2 h-auto bg-gray-100 mb-5 group relative'>
                                    ...
                                    <div className='absolute top-2 right-2'>
                                        <button className='bg-red-500 text-gray-50 px-2 flex items-center justify-center gap-2 py-1 rounded-full h-8 w-8 cursor-pointer hover:bg-red-600' onClick={() => handleRemoveFav(item.OMDBID, item.Title)}>
