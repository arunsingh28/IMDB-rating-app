                ...
                Array.isArray(movieData) && movieData.length > 0 ? (
                    movieData.map((item) => (
                        <div key={item.imdbID} className='w-64 h-96 font-poppins relative group hover:shadow-2xl px-2 py-2 rounded-md hover:h-auto hover:-translate-y-5'>
                            ...
                            {/* fav icon */}
                            <div className='hidden group-hover:block absolute top-2 right-7 duration-200 cursor-pointer' title='Add to favorite list' onClick={() => addToFavList(item)}>
                                <div className='relative'>
                                    <div className='rounded-full h-7 w-7 hover:hidden absolute top-1 -right-[4px] duration-200 text-2xl'>❤️</div>
                                    <button className='cursor-pointer w-8 h-8 text-3xl animate-ping'>❤️</button>
                                </div>
                            </div>
                        </div>
                    ))
